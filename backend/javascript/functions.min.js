...
_default:{lengthMenu:[[10,25,50,100,-1]]}};$(".datatable").each(function(){var b=a[this.id]?a[this.id]:a._default;if(b.lengthMenu&&!b.lengthMenu[1]){b.lengthMenu[1]=[];for(var c=0;c<b.lengthMenu[0].length;c++)b.lengthMenu[1][c]=-1===b.lengthMenu[0][c]?"...":b.lengthMenu[0][c]}this.id&&$("#"+this.id).DataTable(b)})}function modalDlg_setTitle(a=""){document.getElementById("dialogtitle").innerHTML=a?a:"&nbsp;"}function modalDlg_setContent(a=""){document.getElementById("dialogcontent").innerHTML=a}
function showModalUrl(a,b=""){showModal('<iframe src="'+a+'" style="width: 100%; border: 0; height: 800px;"></iframe>',b)}function showModal(a="",b=""){modalDlg_setTitle(b);modalDlg_setContent(a);showModalWindow(!0)}
...
window.addEventListener("load",function(){initDrawH3list();initSoftscroll();initExtendedView();initBase64Input("e_url","urlbase64");initToggleAreas();initDatatables();var a=document.location.pathname.replace(/(.*\/)[a-z0-0\.]*$/,"$1");0<=a.indexOf("/backend/")&&a.indexOf("/backend/")>a.length-10&&(a=document.location.href.replace(/\?.*$/,"").replace(/(.*\/)[a-z0-0\.]*$/,"$1"),updateStatus(a+"/get.php?action=getstatus"))});