...
function initInsertImage(a,b){var c=document.getElementById(a),d=document.getElementById(b);c&&d&&($("body").on("focus","[contenteditable]",function(){const a=$(this);a.data("before",a.html())}).on("paste keyup","[contenteditable]",function(){const a=$(this);a.data("before")!==a.html()&&(a.data("before",a.html()),a.trigger("change"))}),$(c).change(function(){var a=$("#"+this.id+" > img")?$("#"+this.id+" > img").attr("src"):"";a&&d.value!==a&&(d.value=a,c.innerHTML="["+a.length+']<br><img src="'+a+
'">')}))}
...
window.addEventListener("load",function(){initDrawH3list();initSoftscroll();initExtendedView();initToggleAreas();initDatatables();initSelectProject();initInsertImage("profileimageinserter","profileimagedata");initBase64Input("e_url","urlbase64");var a=document.location.pathname.replace(/(.*\/)[a-z0-0\.]*$/,"$1");0<=a.indexOf("/backend/")&&a.indexOf("/backend/")>a.length-10&&(a=document.location.href.replace(/\?.*$/,"").replace(/(.*\/)[a-z0-0\.]*$/,"$1"),updateStatus(a+"/get.php?action=getstatus"))});