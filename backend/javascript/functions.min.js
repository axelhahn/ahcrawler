...
window.addEventListener("load",function(){initDrawH3list();initSoftscroll();initExtendedView();initBase64Input("e_url","urlbase64");initToggleAreas();initDatatables();initSelectProject();var a=document.location.pathname.replace(/(.*\/)[a-z0-0\.]*$/,"$1");0<=a.indexOf("/backend/")&&a.indexOf("/backend/")>a.length-10&&(a=document.location.href.replace(/\?.*$/,"").replace(/(.*\/)[a-z0-0\.]*$/,"$1"),updateStatus(a+"/get.php?action=getstatus"))});