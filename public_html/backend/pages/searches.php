...
$sOldest = $this->oDB->min('searches', ['ts'], [
    'AND' => [
        'siteid' => $this->_sTab,
    ],]);
$sYoungest = $this->oDB->max('searches', ['ts'], [
    'AND' => [
        'siteid' => $this->_sTab,
    ],]);
...
$sReturn .= $oRenderer->renderTileBar($sTiles) . '<div style="clear: both;"></div>'
        . $this->getHistoryCounter(['searches'])
;
...
if (count($aLastSearches)) {
    ...
    $sOptionsLast = '';
    foreach ([5, 10, 20, 50, 100] as $iLastvalue) {
...
$sOptionsTop = '';
foreach ([10, 20, 50, 100] as $iTopvalue) {
 ...
 * 
  ...
  ['query', 'count(query) as count', 'results'],
  [
  'AND' => [
  'siteid' => $this->_sTab,
...
$aSearchterms = ($oResult ? $oResult->fetchAll(PDO::FETCH_ASSOC) : []);
...
if (count($aSearchterms)) {
    ...
    $sReturn .= 
            ...
            . $this->_getSimpleHtmlTable([
                [$this->lB('searches.topn.from'), $sDateFrom],
                [$this->lB('searches.topn.to'), $sDateTo]
            ])
            . '<br>'
